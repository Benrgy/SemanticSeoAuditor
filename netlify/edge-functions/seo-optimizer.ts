...
// Function to get SEO data for dynamic routes (like audit reports)
async function getDynamicSEOData(pathname: string): Promise<SEOData | null> {
  // Handle audit report routes
  if (pathname.startsWith('/audit/')) {
    const auditId = pathname.split('/')[2];
    
    // In a real implementation, you would fetch audit data from Supabase
    // For now, return generic audit report SEO data
    return {
      ...
      canonicalUrl: `https://benrgy.github.io/SemanticSeoAuditor${pathname}`,
      structuredData: {
        "@context": "https://schema.org",
        "@type": "Report",
        "name": "SEO Audit Report",
        "description": "Comprehensive SEO analysis report",
        "url": `https://benrgy.github.io/SemanticSeoAuditor${pathname}`
      }
    };
  }
...
// Function to generate enhanced HTML head
...
// Function to check if request is from a crawler
...
export default async function handler(request: Request, context: Context) {
  ...
  let seoData = routeConfigs[pathname];
  
  // If not found in static configs, try dynamic routes
  if (!seoData) {
    seoData = await getDynamicSEOData(pathname);
  }
  
  // If still no SEO data, use default
  if (!seoData) {
    seoData = routeConfigs['/'];
  }
